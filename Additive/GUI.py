...
from AdditiveEncrup import Encruption,Decription
...
# Create the window
window = sg.Window('caeser Encription ', layout, resizable=True,finalize=True)      
...
while(True):
    ...
    if (event == "Encruption"):
        Enc=Encruption(values[0],int(values[1]))
        window['-OUTPUT-'].update( Enc )
    elif (event == "Decription"):
        Dec=Decription(values[0],int(values[1]))
